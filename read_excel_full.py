...
for label, filepath in files:
    ...
    for row in ws_basic.iter_rows(min_row=2, values_only=False):
        ...
        contact = str(row[contact_idx].value) if row[contact_idx].value and str(row[contact_idx].value) != 'nan' else '未披露'
        phone = str(row[phone_idx].value) if row[phone_idx].value and str(row[phone_idx].value) != 'nan' else '未披露'
        ...
        if bid_num and bid_num in wb.sheetnames:
            ...
            # 详情sheet是竖向结构：第1列是字段名，第2列是值
            for detail_row in ws_detail.iter_rows(min_row=2, values_only=False):
                field_name = str(detail_row[0].value) if detail_row[0].value else ''
                field_value = str(detail_row[1].value) if detail_row[1].value else ''
                
                if '项目正文' in field_name:
                    detail_content = field_value
                elif '招标类型' in field_name:
                    method = field_value
        ...
        all_projects.append({
            ...
            'city': city,
            'method': method,
            'detail_content': detail_content[:800] if detail_content else ''
        })
    
    wb.close()

# 保存结果
with open('/Users/zhouhq/Documents/kimi/workspace/bidding-daily/excel_all_projects_full.json', 'w', encoding='utf-8') as f:
    json.dump(all_projects, f, ensure_ascii=False, indent=2)

print(f"总计读取 {len(all_projects)} 个项目")
print(f"有详情内容的项目: {sum(1 for p in all_projects if p['detail_content'])}")
...
for p in all_projects[:5]:
    if p['detail_content']:
        print(f"\n{p['company']}")
        print(f"  项目: {p['title'][:60]}")
        print(f"  招标方式: {p['method']}")
        print(f"  发布时间: {p['pub_date']}")
